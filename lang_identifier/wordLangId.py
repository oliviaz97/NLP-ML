# word bigram program for language identification with add-one smoothing - Olivia Zhang
import os
import sys
...
for sen in english_sentences:
    sen = sen.split() # split the input by space, store into a list of words
    # get rid of all punctuations in the list of words
    if sen[-1] in string.punctuation:
        sen = sen[:-1]
    sen = ['<s>'] + sen[:-1] + ['<s>'] # concatenate start and end chars for better calculation
...
for sen in french_sentences:
    sen = sen.split() # split the input by space, store into a list
    # get rid of all punctuations in the list of words
    for s in sen:
        if s in string.punctuation:
            sen.remove(s)
    sen = list(sen)
    sen = ['<s>'] + sen + ['<s>'] # concatenate start and end chars for better calculation
